...
TCPSegment::TCPSegment(void *buffer) {
    setBuffer(buffer);
}
...
unsigned short TCPSegment::getSrcPort() {
    return htons(header->srcPort);
}
void TCPSegment::setSrcPort(unsigned short srcPort) {
    header->srcPort = htons(srcPort);
}
unsigned short TCPSegment::getDestPort() {
    return htons(header->destPort);
}
...
bool TCPSegment::isSYN() {
    return htons(header->length_flag) & 0x0002;
}

bool TCPSegment::isFIN() {
    return htons(header->length_flag) & 0x0001;
}
