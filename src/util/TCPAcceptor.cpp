...
TCPStream *TCPAcceptor::accept() {
    ...
    while (true) {
        ...
        ssize_t segmentSize = SegmentLoader::catchSegment(
            socketDescriptor, buffer, _le_port, _le_address, srcPort, srcIP);
        if (segmentSize == -1) {
            delete buffer;
            return nullptr;
        }
        ...
        if (!(!segment.isACK() && segment.isSYN() && !segment.isFIN()))
            continue;
        ...
        segmentSize = SegmentLoader::sendSegment(socketDescriptor, srcIP,
                                                 buffer, sizeof(TCPHeader));

        //  await the thrid handshake
        segmentSize = SegmentLoader::catchSegment(
            socketDescriptor, buffer, _le_port, _le_address, srcPort, srcIP);
